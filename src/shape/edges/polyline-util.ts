...
import { IShapeBase, IBBox } from '../../types';
...
export const getBBoxFromPoint = (point: PolyPoint): IBBox => {
  const {
    x,
    y
  } = point;
...
export const getBBoxFromPoints = (points: PolyPoint[] = []): IBBox => {
  const xs = [];
  const ys = [];
  points.forEach(p => {
    xs.push(p.x);
...
export const isBBoxesOverlapping = (b1: IBBox, b2: IBBox) => {
  return Math.abs(b1.centerX - b2.centerY) * 2 < (b1.width + b2.width) && Math.abs(b1.y - b2.y) * 2 < (b1.height + b2.height);
};
...
export const getExpandedBBox = (bbox, offset: number): IBBox => {
...
export const isHorizontalPort = (port: PolyPoint, bbox: IBBox): boolean => {
 ...
 * @param b2 
 */
export const mergeBBox = (b1: IBBox, b2: IBBox): IBBox => {
...
export const getPointsFromBBox = (bbox: IBBox): PolyPoint[] => {
  // anticlockwise
  const {
    minX,
    minY,
...
export const isPointOutsideBBox = (point: PolyPoint, bbox: IBBox): boolean => {
  const {
    x,
    y
  } = point;
...
export const getBBoxXCrossPoints = (bbox: IBBox, x: number): PolyPoint[] => {
...
export const getBBoxYCrossPoints = (bbox: IBBox, y: number): PolyPoint[] => {
...
export const getBBoxCrossPointsByPoint = (bbox: IBBox, point: PolyPoint): PolyPoint[] => {
...
export const isSegmentCrossingBBox = (p1: PolyPoint, p2: PolyPoint, bbox: IBBox): boolean => {
...
export const getPolylinePoints = (start: PolyPoint, end: PolyPoint, sNode: IShapeBase, tNode: IShapeBase, offset: number): PolyPoint[] => {
  ...
  const centerPoint = {
    x: (start.x + end.x) / 2,
    y: (start.y + end.y) / 2
  }; [ lineBBox, sMixBBox, tMixBBox ].forEach((bbox: IBBox) => {
